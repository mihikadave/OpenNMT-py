...
if [[ I -eq 0 ]]; then
  for index in 0 1
  do
  	python train.py -data data/${TRAIN_DATA[$index]} -save_model trained_models/${MODEL[$index]} -gpuid $GPU -encoder_type $ENCODER -decoder_type $DECODER -dropout $DROPOUT
  done
fi
